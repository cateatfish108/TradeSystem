...
QString hyarray[600][4];   //定义一个合约的二纬数组
int k=0;
...
{
	//	if (g_pTradeUserApi == NULL) { return 0; }
	CThostFtdcReqAuthenticateField  req; //认证
	//认证码
	memset(&req, 0, sizeof(CThostFtdcReqAuthenticateField));
	strcpy(req.BrokerID, jy.BROKER_ID);  //模拟经纪商代码
	strcpy(req.UserID, jy.INVESTOR_ID);//账户名
	///strcpy(req.UserProductInfo, "Quant_test1"); //产品标识 已弃用
	strcpy(req.AuthCode, jy.AUTHCODE);  //认证码
	strcpy(req.AppID, jy.APPID);  //APP编号
	int iResult = pUserApi->ReqAuthenticate(&req, ++iRequestTdID);
	...
	bool bIsLast)
{
	...
	{
		//printf("认证成功,ErrorID=0x%04x, ErrMsg=%snn", pRspInfo->ErrorID, pRspInfo->ErrorMsg);
		ReqUserLogin();
	}
	else
	{
